...
// ContainerRestore takes configs and container ID to restore a container.
func (daemon *Daemon) ContainerRestore(params *ContainerCreateConfig, id string) (types.ContainerCreateResponse, error) {
	if params.Config == nil {
		return types.ContainerCreateResponse{}, derr.ErrorCodeEmptyConfig
	}
	...
	container, err := daemon.restoreContainer(params, id)
	if err != nil {
		return types.ContainerCreateResponse{ID: "", Warnings: warnings}, daemon.graphNotExistToErrcode(params.Config.Image, err)
	}
...
// restoreContainer restore a container from the given configuration and container id
func (daemon *Daemon) restoreContainer(params *ContainerCreateConfig, id string) (retC *Container, retErr error) {
	var (
		container *Container
		img       *image.Image
		imgID     string
	...
	if container, err = daemon.restoreNewContainer(params.Name, params.Config, imgID, id); err != nil {
		return nil, err
	}
	defer func() {
		if retErr != nil {
