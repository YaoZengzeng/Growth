...
func (ctx *VmContext) onVmStop(reclaim bool) bool {
	glog.V(1).Info("VM has stopped...")
	ctx.handler = nil
	return true 
}
...
func (ctx *VmContext) migratePod(cmd *MigratePodCommand) {
	//ctx.setTimeout(30)
/*	ctx.vm <- &DecodedMessage {
		Code:		INIT_MIGRATEPOD,
		Message:	[]byte{},
	}*/
	ctx.DCtx.Migrate(ctx, cmd.Ip, cmd.Port)
}
...
func commonStateHandler(ctx *VmContext, ev VmEvent, hasPod bool) bool {
	...
	case EVENT_VM_STOP:
		glog.Info("GOT VM stop event, go back")
		ctx.unsetTimeout()
		if closed := ctx.onVmStop(hasPod); !closed {
			glog.Info("Not closing migrated Vm succeed")
		} else {
			glog.Info("Closing migrated Vm succeed")
		}
...
func unexpectedEventHandler(ctx *VmContext, ev VmEvent, state string) {
	fmt.Println("get event: ",EventString(ev.Event()))
	switch ev.Event() {
...
func stateRunning(ctx *VmContext, ev VmEvent) {
	if processed := commonStateHandler(ctx, ev, true); processed {
		...
		if ctx.deviceReady () {
			glog.V(1).Info("devices ready before migration")
		}
	} else {
		switch ev.Event() {
		...
		case COMMAND_MIGRATE_POD:
			ctx.migratePod(ev.(*MigratePodCommand))
		case COMMAND_RELEASE:
